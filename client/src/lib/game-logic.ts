...
export function calculateCaptures(board: BoardState, position: Position): Position[] {
  ...
  for (const [dr, dc] of directions) {
    ...
    // Check if adjacent piece is an enemy
    const isEnemyPiece =
      (piece === 'attacker' && (adjacentPiece === 'defender' || adjacentPiece === 'king')) ||
      ((piece === 'defender' || piece === 'king') && adjacentPiece === 'attacker')

    if (!isEnemyPiece) continue

    // King has special capture rules - skip regular capture logic
...
export function checkWinCondition(board: BoardState): {
  ...
  if (
    (kingPosition.row === 0 && kingPosition.col === 0) ||
...
function isKingSurrounded(board: BoardState, kingPos: Position): boolean {
    ...
    for (const [dr, dc] of directions) {
      ...
      if (newRow >= 0 && newRow < 11 && newCol >= 0 && newCol < 11) {
        ...
        if (board[newRow][newCol] === 'attacker' || (newRow === 5 && newCol === 5)) {
          // Throne counts as attacker for capture
          surroundedSides++
        }
      } else {
